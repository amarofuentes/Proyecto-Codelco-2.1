...
function drawChart(series, p) {
  ...
  for (let i = 0; i <= 5; i++) {
    ...
    ctx.stroke();
    ctx.fillText(yVal.toFixed(1), 20, y + 4);
  }
  ...
  ctx.fillStyle = '#445b86';
  ctx.font = '13px Inter';
  ctx.fillText('Nivel de costo', 16, pad.top - 2);
  ctx.fillText('Grado de integración', W / 2 - 65, H - 10);
  ...
  const points = [
    { y: yPos(selected.ct), color: '#2563c9' },
    { y: yPos(selected.cc), color: '#e24a4a' },
    { y: yPos(selected.total), color: '#17a27c' }
  ];
  points.forEach(pt => {
    ctx.beginPath();
  ...
  ctx.beginPath();
  ctx.arc(optX, yPos(series.minTotal), 5.5, 0, Math.PI * 2);
  ctx.fillStyle = '#d6a11f';
  ctx.fill();

  // subtle selected line
  ctx.setLineDash([4,5]);
  ctx.strokeStyle = 'rgba(37,99,201,.35)';
  ctx.beginPath();
